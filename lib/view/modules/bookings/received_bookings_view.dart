...
class _ReceivedBookingsViewState extends ConsumerState<ReceivedBookingsView> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(fetchReceivedBookingsProvider);
    });
  }
  ...
  Widget build(BuildContext context) {
    final receivedBookingsAsync = ref.watch(fetchReceivedBookingsProvider);
              ...
              receivedBookingsAsync.when(
                data: (bookings) {
                  final filtered = bookings.where((b) {
                    final matchesSearch = b.recruiter?.name?.toLowerCase().contains(searchQuery) ?? false;
